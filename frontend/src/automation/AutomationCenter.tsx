...
export function AutomationCenter(props: AutomationCenterProps) {
  ...
  return <div className="zone-page automation-zone">
    <section className="zone-hero compact-hero">
      ...
      <StatusBadge label={activeFaults.length ? `${activeFaults.length} PLC Fault${activeFaults.length === 1 ? "" : "s"}` : "Controllers Online"} state={activeFaults.length ? "offline" : "online"}/>
    </section>
