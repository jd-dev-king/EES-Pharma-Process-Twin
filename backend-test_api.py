...
def test_batch_termination_creates_hold_genealogy_and_qa_task(client):
    ...
    released=client.post(f"/api/quality/bulk-tasks/{tasks[0]['task_id']}/qa-disposition",json={"disposition":"Release","note":"Meets specification"})
...
def release_bulk_for_packaging(client, po_number="PO-PKG-01", packaging_line="PKG-01"):
    ...
    client.post(f"/api/quality/bulk-tasks/{task['task_id']}/qa-disposition",json={"disposition":"Release","note":"Bulk approved"})
...
def test_packaging_run_and_fg_release_create_outbound_to(client, monkeypatch):
    ...
    release=client.post(f"/api/quality/fg-tasks/{task['task_id']}/qa-disposition",json={"disposition":"Release","note":"FG meets specification"})
...
def test_bulk_receiving_and_pg_transfer(client):
    register(client, "PO-BULK-01")
    created = client.post("/api/bulk/deliveries", json={
        "material_code":"PG","material_name":"Propylene Glycol","vendor":"Global Pharma Glycols",
        "carrier":"Vendor Tanker","quantity_kg":5000,"receiving_bay":"BAY-01","tank_code":"PG-101",
        "delivery_window":"08:00-10:00","lot_number":"PG-LOT-01","coa_number":"COA-01",
        "seal_number":"SEAL-01","temperature_c":22
    })
    assert created.status_code == 201, created.text
    delivery_id = created.json()["delivery_id"]
    inspected = client.post(f"/api/bulk/deliveries/{delivery_id}/inspect", json={
        "tanker_verified":True,"material_verified":True,"seal_intact":True,"coa_verified":True,"temperature_accepted":True
    })
    assert inspected.status_code == 200
    assert client.post(f"/api/bulk/deliveries/{delivery_id}/actions/start").status_code == 200
    for _ in range(5):
        result = client.post(f"/api/bulk/deliveries/{delivery_id}/actions/tick")
    assert result.json()["status"] == "Awaiting QA Sample"
    assert client.post(f"/api/bulk/deliveries/{delivery_id}/actions/sample").status_code == 200
    released = client.post(f"/api/bulk/deliveries/{delivery_id}/qa-disposition", json={"disposition":"Release"})
    assert released.status_code == 200 and released.json()["status"] == "Released"
    tanks = client.get("/api/bulk/tanks").json()
    assert next(t for t in tanks if t["tank_code"] == "PG-101")["qa_status"] == "Released"
    transfer = client.post("/api/bulk/transfers", json={"po_number":"PO-BULK-01","source_tank":"PG-101","destination_tank":"V-201","quantity_kg":420,"operator":"Process Operator"})
    assert transfer.status_code == 201, transfer.text
    transfer_id = transfer.json()["transfer_id"]
    assert client.post(f"/api/bulk/transfers/{transfer_id}/verify", json={"identity_verified":True,"qa_release_verified":True,"hose_connected":True}).status_code == 200
    assert client.post(f"/api/bulk/transfers/{transfer_id}/start").status_code == 200
    for _ in range(5):
        result = client.post(f"/api/bulk/transfers/{transfer_id}/tick")
    assert result.json()["status"] == "Complete"


def test_bulk_delivery_requires_pre_unload_qa_release(client):
    created = client.post("/api/bulk/deliveries", json={
        "material_code": "PG", "material_name": "Propylene Glycol",
        "vendor": "Global Pharma Glycols", "carrier": "Vendor Tanker",
        "quantity_kg": 5000, "receiving_bay": "BAY-01", "tank_code": "PG-101",
        "delivery_window": "08:00-09:00", "lot_number": "PG-TEST-001",
        "coa_number": "COA-PG-001", "seal_number": "SEAL-001", "temperature_c": 22
    })
    assert created.status_code == 201
    delivery_id = created.json()["delivery_id"]

    blocked = client.post(f"/api/bulk/deliveries/{delivery_id}/actions/start")
    assert blocked.status_code == 409

    inspected = client.post(f"/api/bulk/deliveries/{delivery_id}/inspect", json={
        "tanker_verified": True, "material_verified": True, "seal_intact": True,
        "coa_verified": True, "temperature_accepted": True
    })
    assert inspected.status_code == 200
    assert inspected.json()["status"] == "Ready for Pre-Unload Sample"

    sampled = client.post(f"/api/bulk/deliveries/{delivery_id}/actions/sample")
    assert sampled.status_code == 200
    assert sampled.json()["status"] == "Pending QA Review"

    still_blocked = client.post(f"/api/bulk/deliveries/{delivery_id}/actions/start")
    assert still_blocked.status_code == 409

    released = client.post(f"/api/bulk/deliveries/{delivery_id}/qa-disposition", json={"disposition": "Release"})
    assert released.status_code == 200
    assert released.json()["status"] == "QA Released for Unloading"

    started = client.post(f"/api/bulk/deliveries/{delivery_id}/actions/start")
    assert started.status_code == 200
    assert started.json()["status"] == "Unloading"
    ...
    tank = next(item for item in client.get("/api/bulk/tanks").json() if item["tank_code"] == "PG-101")
    assert tank["quantity_kg"] == 5000
    assert tank["qa_status"] == "Released"


def test_rejected_bulk_delivery_never_updates_tank_inventory(client):
    before = next(item for item in client.get("/api/bulk/tanks").json() if item["tank_code"] == "PG-101")["quantity_kg"]
    created = client.post("/api/bulk/deliveries", json={
        "material_code": "PG", "material_name": "Propylene Glycol",
        "vendor": "Rejected Vendor", "carrier": "Vendor Tanker",
        "quantity_kg": 2500, "receiving_bay": "BAY-01", "tank_code": "PG-101",
        "delivery_window": "10:00-11:00", "lot_number": "PG-REJECT-001",
        "coa_number": "COA-REJECT", "seal_number": "SEAL-REJECT", "temperature_c": 22
    })
    delivery_id = created.json()["delivery_id"]
    client.post(f"/api/bulk/deliveries/{delivery_id}/inspect", json={
        "tanker_verified": True, "material_verified": True, "seal_intact": True,
        "coa_verified": True, "temperature_accepted": True
    })
    client.post(f"/api/bulk/deliveries/{delivery_id}/actions/sample")
    rejected = client.post(f"/api/bulk/deliveries/{delivery_id}/qa-disposition", json={"disposition": "Reject"})
    assert rejected.status_code == 200 and rejected.json()["status"] == "Rejected"
    assert client.post(f"/api/bulk/deliveries/{delivery_id}/actions/start").status_code == 409
    after = next(item for item in client.get("/api/bulk/tanks").json() if item["tank_code"] == "PG-101")["quantity_kg"]
    assert after == before
